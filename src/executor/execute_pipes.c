...
{	
	...
	{//erverthing frome here is executed in parent procces
		...
		shell.pipes.output_len = read(shell.pipes.pipe_fd[0],shell.pipes.output_str,sizeof(10));
		shell.pipes.output_str[shell.pipes.output_len] = '\0';
		
		if(shell.pipes.output_len == -1)
			printf("read failed\n");
		
    	
		printf("read: %d: %s\n", shell.pipes.output_len, shell.pipes.output_str);
